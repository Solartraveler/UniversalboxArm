...
//should be a block within the size of the SD card
...
static bool CardInitWithReadCheck(void) {
	uint8_t buffer[SDMMC_BLOCKSIZE];
	if (CardInit()) {
		if (SdmmcWriteSingleBlock(buffer, READ_BLOCK_CHECK)) {
			return true;
		}
	}
	return false;
...
/*Call with dataOffset = 0 to restart the internal state machine.
  ...
  7 card reported CC error, try next command
...
void LoadSettings(void) {
	...
	if (FilesystemReadFile(CONFIG_FILENAME, json, sizeof(json), &readLen)) {
		...
		LoadValueUint32(json, readLen, "arg", &g_testArg, 0, 0xFFFFFFFF, 1);
...
void IncrementSet(void) {
	CoprocWatchdogReset();
	printf("Enter increment (must be at least 1)\r\n");
	char buffer[16] = {0};
	ReadSerialLine(buffer, sizeof(buffer));
	unsigned int arg = 0;
	sscanf(buffer, "%u", &arg);
