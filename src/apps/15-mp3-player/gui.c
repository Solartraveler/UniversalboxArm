...
void GuiUpdateFilelist(void) {
	...
	if (f_opendir(&d, g_gui.filepath) == FR_OK) {
		while (f_readdir(&d, &fi) == FR_OK) {
			if (fi.fname[0]) {
				printf("%s\r\n", fi.fname);
				if ((EndsWith(fi.fname, ".mp3")) || (fi.fattrib & AM_DIR)) {
					if (elementBefore) {
						GuiAppendFilelist("\n");
					}
...
void GuiInit(void) {
	...
	if (g_gui.type != NONE) {
		LcdBacklightOn();
		/*Time for menu_screen_flush @ 80MHz CPU, screen scale factor 2x2 to
		  320x240 pixel and SPI clock of.
		         two buff:   single buff:
		  10MHz: 129ms       144ms
		  20MHz:  67ms        82ms
		  40MHz:  37ms        52ms
		*/
		LcdEnable(4); //20MHz SPI @ 80MHz CPU clock
		LcdInit(g_gui.type);
		if (g_gui.type == ILI9341) {
			g_gui.pixelX = 320;
			g_gui.pixelY = 240;
		} else {
			printf("LCD unsupported\r\n");
		}
	} else {
		printf("LCD not enabled\r\n");
	}
	menu_screen_scale(0, 0, 2, 2);
	menu_redraw();
}
...
void GuiCycle(char key) {
	...
	g_gui.cycle++;
	/* Split up to different cycles to shorten each cycle call, so the mp3 decoding
	   is processed with less maximum waiting time again.
	*/
	if (g_gui.cycle == 400) {
		GuiUpdateText();
	}
