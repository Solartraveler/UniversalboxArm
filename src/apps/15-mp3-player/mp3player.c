...
/*Dummy functions to save memory. They are referenced by libmad, but are not called
  in the setup this app is providing for the lib.
*/
...
void PlayerMp3Cycle(void) {
	...
	while (1) {
		if (pMad->header_func) {
			...
			switch (pMad->header_func(pMad->cb_data, &frame->header)) {
				case MAD_FLOW_STOP:
				case MAD_FLOW_BREAK:
					break;
...
void AppCycle(void) {
	...
	GuiCycle(input); //processes on the 400 and 500th call
	StackSampleCheck();
