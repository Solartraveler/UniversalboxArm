...
*/
uint8_t ImgCompressed1ByteLowres(uint8_t x, uint8_t y, uint8_t colorBits,
              uint8_t * outBuffer, size_t outBufferLen, uint8_t backgroundColor,
              Img1BytePixelLowres_t * drawPixels, uint16_t numPixel, size_t * outBufferUsed);
...
*/
uint16_t ImgInterpolateToPixelsLowres(const uint16_t * dataIn, uint16_t dataPointsIn,
               Img1BytePixelLowres_t * drawPixels, uint16_t maxPixels, uint8_t color);
...
tempBuffer: Optional processing buffer, for each pixel drawn, one Img1BytePixelLowres_t needs to be
            temporary stored. If NULL, y * 10 * sizeof(Img1BytePixelLowres_t) bytes are allocated on the stack.
...
*/
uint8_t ImgCreateLineGfx1BitLowres(const uint16_t * data, uint16_t dataPoints, uint8_t x, uint8_t y, bool flip,
...
outGfxUsed: Output, if non NULL, used length of compressedGfxOut
tempBuffer: Optional processing buffer, for each pixel drawn, one Img1BytePixelLowres_t needs to be
        temporary stored. If NULL, y * lines 10 * sizeof(Img1BytePixelLowres_t) bytes are allocated on the stack.
tempBufferLen: Number of bytes buffer has. May be 0.
returns: 0: successful
         1: ok, but data are cut out due lack of memory
         2: compressedGfxOut too small, may not be used.
*/
uint8_t ImgCreateLinesGfxLowres(const uint16_t ** data, uint16_t dataPoints, const uint8_t * colors, uint8_t lines,
              uint8_t x, uint8_t y, uint8_t colorBits, bool flip,
              uint8_t * compressedGfxOut, size_t outGfxLen, size_t * outGfxUsed, void * tempBuffer, size_t tempBufferLen);
